...
func _create_call(call_data):
	...
	for index in call_data.transcript.size():
		...
		if existing_label != null:
			...
			current_y += Constants.SPACING_BETWEEN_MESSAGE
			
	call_holder.custom_minimum_size.y = Constants.SPACING_BETWEEN_MESSAGE * call_data.transcript.size()
...
func _typewrite_effect(label: RichTextLabel):
	...
	for character in total_characters:
		...
		await get_tree().create_timer(Constants.TYPEWRITE_EFFECT_SPEED).timeout
...
func _on_call_accepted():
	for label in call_holder.get_children():
		...
		label.text = "(%s) %s" % [GameCycle.get_timestamp(), label.text]
		
		await _typewrite_effect(label)
		timer.start(info.wait_time if info.has("wait_time") else Constants.TIME_BETWEEN_MESSAGE)
		await timer.timeout
